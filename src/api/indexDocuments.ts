...
import { trackDocumentIndexing, trackFileUpload, Events, trackEvent } from '@/lib/monitoring';
...
export async function indexDocument(
  ...
  try {
    ...
    return await trackDocumentIndexing(projectId, 1, async () => {
      // Get current user
      ...
      // First upload file to storage with category - track separately
      const uploadResult = await trackFileUpload(file.size, file.type, async () => {
        return await uploadFileToStorage(projectId, file, user.id, category);
      });
      ...
      trackEvent(Events.DOCUMENT_UPLOADED, 'info', {
        projectId,
        fileName: file.name,
        fileSize: file.size,
        category
      ...
      trackEvent(Events.DOCUMENT_INDEXED, 'info', {
        projectId,
